...
- **Components** — 22 published under [`actpkg.dev/library`](https://actpkg.dev):
  - *Data & storage* — `sqlite`, `postgres`, `filesystem`, `archive`
  - *Network* — `http-client`, `search-brave`, `search-searxng`
  - *Documents* — `anydoc`, `pdf-inspector`
  - *Compute* — `python-env` (numpy + pandas), `python-eval`, `sed`
  - *Browser & desktop* — `webdriver-bidi`, `vnc-desktop`
  - *Bridges* — `mcp-bridge`, `openapi-bridge`, `act-http-bridge`
